...
//################  VERSION  ###########################################
String    Version = "1.00";   // Programme version
//################ VARIABLES ###########################################

// WEBSERVER VARIABLE DEFINITION
typedef struct
{
  String filename;
  String ftype;
...
//  while (!Serial);
  ...
  int counterForWifiConnect = 0;
  while (WiFi.status() != WL_CONNECTED) 
  {
    delay(500);
    Serial.print(".");
    counterForWifiConnect++;
    //if (sanity > 5) break;
    if (counterForWifiConnect > 10) ESP.restart();
  }
  ...
  {
    request->send(FS, "/icon5.png", "image/png");
  });
...
{ // Rename the file
  ...
  {
    webpage += "<tr><td><input type='text' name='oldfile' style='color:black;' value = '" + Filenames[index].filename + "' readonly></td>";
...
void Handle_File_Download()
{
  ...
  webpage += "<table>";
  webpage += "<tr><th>File Name</th><th style='text-align: right;'>File Size</th></tr>";
  while (index < numfiles) 
  {
    webpage += "<tr><td><a href='" + Filenames[index].filename + "'></a><td style='text-align: right;'>" + Filenames[index].fsize + "</td></tr>";
    index++;
  }
  webpage += "</table>";
...
void Select_File_For_Function(String title, String function)
{
  ...
  webpage += "<table class='center'>";
  webpage += "<tr><th>File Name</th><th style='text-align: right;'>File Size</th><th class='sp'></th><th>File Name</th><th style='text-align: right;'>File Size</th></tr>";
  while (index < numfiles) 
  {
    ...
    webpage += "<tr>";
    webpage += "<td style='width:25%;'><button><a href='" + function + "~/" + Fname1 + "'>" + Fname1 + "</a></button></td><td style = 'width:10%; text-align: right;'>" + Filenames[index].fsize + "</td>";
    webpage += "<td class='sp'></td>";
    if (index < numfiles - 1) 
    {
      webpage += "<td style='width:25%;'><button><a href='" + function + "~/" + Fname2 + "'>" + Fname2 + "</a></button></td><td style = 'width:10%; text-align: right;'>" + Filenames[index + 1].fsize + "</td>";
    }
    webpage += "</tr>";
    index = index + 2;
  }
...
void Display_File_List()
{
  ...
  Directory(); // Get a list of files on the FS
  webpage = HTML_Header();
  webpage += "<h3>List Files in this Device</h3>";
  webpage += "<table class='center'>";
  webpage += "<tr><th>File Name</th><th style='text-align: right;'>File Size</th><th class='sp'></th><th>File Name</th><th style='text-align: right;'>File Size</th></tr>";
  while (index < numfiles) 
  {
    ...
    if (Fname2.startsWith("/")) Fname1 = Fname2.substring(1);
    webpage += "<tr>";
    webpage += "<td style='width:25%'>" + Fname1 + "</td><td style = 'width:10%; text-align: right;'>" + Filenames[index].fsize + "</td>";
    webpage += "<td class='sp'></td>";
    if (index < numfiles - 1) 
    {
      webpage += "<td style='width:25%'>" + Fname2 + "</a></button></td><td style = 'width:10%; text-align: right;'>" + Filenames[index + 1].fsize + "</td>";
    }
    webpage += "</tr>";
    index = index + 2;
  }
...
void Home()
{
  ...
  webpage += "<h2>Christian's ESP32 SmartLamp Webserver</h2>";
  webpage += "<img src = 'icon' alt='icon'>";
  webpage += "<h3>File Management - Directory, Upload, Download, Stream and Delete File</h3>";
  webpage += HTML_Footer();
}
...
void Display_System_Info()
{
  ...
  webpage += "<tr><td>" + ConvBinUnits(uploadsize, 1) + "</td><td>" + ConvBinUnits(downloadsize, 1) + "</td><td style='text-align: right;'>File Size</td></tr> ";
  webpage += "<tr><td>" + ConvBinUnits((float)uploadsize / uploadtime * 1024.0, 1) + "/Sec</td>";
  webpage += "<td>" + ConvBinUnits((float)downloadsize / downloadtime * 1024.0, 1) + "/Sec</td><td style='text-align: right;'>Transfer Rate</td></tr>";
  webpage += "</table>";
  ...
  webpage += "<tr><td>WiFi SSID</td><td>"                   + String(ssid) + "</td></tr>";
...
String HTML_Header()
{
  ...
  page += "<html lang = 'en'>";
  page += "<head>";
  page += "<title>ESP32 SmartLamp Web Server</title>";
  page += "<meta charset='UTF-8'>"; // Needed if you want to display special characters like the ° symbol
  page += "<style>";
  page += "body {width:75em;margin-left:auto;margin-right:auto;font-family:Arial,Helvetica,sans-serif;font-size:16px;color:black;background-color:#fdffe1;text-align:center;}";
  page += "footer {padding:0.08em;background-color:#ffe0a5;font-size:1.1em;}";
  ...
  page += "tr:nth-child(even) {background-color:#fdffe1;}";
  page += "h4 {color:#9a6d1f;font:0.8em;text-align:left;font-style:oblique;text-align:center;}";
  page += ".center {margin-left:auto;margin-right:auto;}";
  page += ".topnav {overflow: hidden;background-color:#fffce0;}";
  page += ".topnav a {float:center;color:black;text-align:center;padding:0.6em 0.6em;text-decoration:none;font-size:1.3em;}";
  page += ".topnav a:hover {background-color:#e99401;color:white;}";
  page += ".topnav a.active {background-color:#e6dbad;color:black;}";
...
//  page += "<a href='/json/info'>[JSON Info]</a>";
...
String HTML_Footer()
{
  String page;
  page += "<br><br><footer>";
  page += "<p class='medium'>Christian's ESP32 SmartLamp Webserver</p>";
...
//######EOF WEBSERVER MANAGEMENT COMMANDS #######//
